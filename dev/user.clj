 (ns user
   (:require [reloaded.repl :refer [set-init! system init start stop go reset reset-all]]
             ...
             [org.arachne-framework.template.clojure-remote :as app])
   )
...
(defn init-arachne
  ...
  [runtime]
  (let [cfg (a/config :org.arachne-framework.template/clojure-remote)
...
(comment
  ...
  (def cfg (a/config :org.arachne-framework.template/clojure-remote))
  (def rt (a/runtime cfg :org.arachne-framework.template.clojure-remote/runtime))
